...
function wpcp_init() {
	...
	// Add thickbox capability to plugin
	wpcp_addthickbox();
}

// Enables thickbox capabilities
function wpcp_addthickbox() {
	add_thickbox();
}
...
function wpcp_warning($wpcp_warning_message) {
	$options = get_option( 'circupress-account' );
	if( $options['wpcp_apikey'] =='' ) {
		echo "<div id='wpcp-warning' class='error fade'><p>";
		echo __( 'CircuPress is almost ready.' );
		echo sprintf(__( 'You must <strong><a href="%1$s">enter your API Key</a></strong> for it to work.' ), "edit.php?post_type=email&page=circupress-account");
		echo "</strong></p></div>";
	}
	if (isset($wpcp_warning_message) && !empty($wpcp_warning_message)) {
		echo "<div id='wpcp-warning' class='error fade'><p>";
		echo __( 'CircuPress is unavailable.' );
		echo __( 'Error: <strong>.' );
		echo $wpcp_warning_message;
		echo "</strong></p></div>";
	}
}
...
function codex_add_help_text( $contextual_help, $screen_id, $screen) {
  ...
  if ( 'email' == $screen->id ) {
    $contextual_help =
      ...
      '<p>' . __( '<a href="https://circupress.com" target="_blank">Documentation</a>' ) . '</p>' .
      '<p>' . __( '<a href="https://circupress.com" target="_blank">Support</a>' ) . '</p>' ;
...
function wpcp_settings_desc_callback(){
	echo "<p>When you successfully sign up for <a href=\"https://www.circupress.com/admin\" target=\"_blank\">CircuPress</a>, we provide you with this API Key to begin sending emails!</p>";
}
...
function wpcp_init_feed_monthly(){
	// Initialize the Feed
	add_feed( 'circupress-monthly','wpcp_email_create' );
}
...
function wpcp_build_social( $wpcp_account_options, $float = "center"){
	...
	$wpcp_icon = 'https://circupress.com/wp-content/uploads/2016/03/webicon-';
	...
	if( strlen( $wpcp_google_plus ) > 0 ){
		$html .= '<td width="10">&nbsp;</td>';
		$html .= '<td width="35"><a href="'.$wpcp_google.'" target="_blank"><img src="'.$wpcp_icon.'googleplus.png" alt="Google" style="border:none;" /></a></td>';
	}
...
// Remove hidden tags that may break the feed
function wpcp_clean_tags($wpcp_content_post) {
    $wpcp_content_post = preg_replace(
        array(
...
function wpcp_email_create(){
	...
	if( $wpcp_post_title != 'NO POSTS' && isset( $_GET['make_post'] ) && strlen( $_GET['make_post'] ) > 0 && $_GET['make_post'] == 1 ){
		...
		$processedHTML = wpcp_clean_tags($wpcp_content_post);
...
function wpcp_admin_footer(){
	...
	echo '<iframe src="https://circupress.com/wp-content/themes/circupress/analytics.php?p='.$p.'" scrolling="no" frameborder="0" width="1" height="1"></iframe>';
